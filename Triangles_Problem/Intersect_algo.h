...
int Get_extreme_ind(const Triangle& tr, const Vec& pt)
{
    ...
    while (true)
    {
        ...
        Vec E(tr.get_vec(next) - tr.get_vec(mid));
        ...
        {
            int prev = (mid + 3 - 1) % 3;
            E = tr.get_vec(mid) - tr.get_vec(prev);
...
bool Triangle::Trian_intersection(const Triangle& trian1, const Triangle& trian2)
{
    for (int i0 = 0, i1 = 2; i0 < 3; i1 = i0, ++i0)
    {
        Vec D1((trian1.get_vec(i0) - trian1.get_vec(i1)).perp2D()), D2((trian2.get_vec(i0) - trian2.get_vec(i1)).perp2D());

        int min1 = Get_extreme_ind(trian1, -D1), min2 = Get_extreme_ind(trian2, -D2);

        Vec diff1(trian2.get_vec(min1) - trian1.get_vec(i0)), diff2(trian1.get_vec(min2) - trian2.get_vec(i0));
...
double Ind_of_max(double a, double b, double c)
{
    int ind = 0;
    double max;
...
bool Triangle::Intersect_2D(const Triangle& trian1, const Triangle& trian2)
{
    Vec Norm(0, 0, 0);
    ...
    for (size_t i = 0; i < 2; ++i)
    {
        ...
        tr1[0][j] = trian1.vec1[i];
        tr2[0][j] = trian2.vec1[i];
        tr1[1][j] = trian1.vec2[i];
        tr2[1][j] = trian2.vec2[i];
        tr1[2][j] = trian1.vec3[i];
        tr2[2][j] = trian2.vec3[i];
        ++j;
    }
    return Trian_intersection(Triangle(tr1[0], tr1[1], tr1[2]), Triangle(tr2[0], tr2[1], tr2[2]));
}
*/
